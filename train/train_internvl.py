...
from utils.datamodule_internvl import DataModuleCustom
...
def train(args, infer_pg, is_infer_rank, train_pg, group_rank, infer_rank,teacher,paths):
    ...
    # tokenizer_mllm = AutoProcessor.from_pretrained(paths, trust_remote_code=True)
    tokenizer_mllm = AutoTokenizer.from_pretrained(paths, trust_remote_code=True,pad_token='<|endoftext|>')
    proj_t5 = create_proj_internvl4b(in_channels=37, use_t5=False, use_scale=False, use_cnn=True).to(dtype=weight_dtype)
...
def infer(args, infer_pg, is_infer_rank, infer_rank,teacher,paths):
    ...
    text_encoder_t5 = InternVLChatModel.from_pretrained(paths, torch_dtype=torch.bfloat16)
    ...
    while True:
        with torch.no_grad():
            ...
            output_hidden_state_all = text_encoder_t5.generate(pixel_values = None, input_ids=input_ids_t5.to(infer_device), attention_mask=attention_mask.to(infer_device))
...
def main(args):
    ...
    paths = "/mnt/data/group/models/InternVL2_5-4B"
