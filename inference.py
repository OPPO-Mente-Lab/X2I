...
minicpm_proj_path = f"./checkpoints/minicpm/{exp_type}/diffusion_pytorch_model.bin"
...
def get_proj(proj_path, exp_type=exp_type):
    in_channels = int(exp_type.split("_")[0])
    use_t5 = "t5" in exp_type
    use_scale = "scale" in exp_type
    use_cnn = "cnn" in exp_type
    proj = create_proj3(in_channels=in_channels, use_t5=use_t5, use_scale=use_scale, use_cnn=use_cnn)
...
minicpm_proj, minicpm_in_channels = get_proj(minicpm_proj_path, exp_type=exp_type)
...
def get_text_embeddings(output_hidden_state, in_channels=minicpm_in_channels):
    text_embeddings = torch.stack(output_hidden_state, dim=1)
    if in_channels == 1:
        text_embeddings = text_embeddings[:,-1:].contiguous()
    elif in_channels == 2:
        text_embeddings_0 = text_embeddings[:,0:1]
        text_embeddings_1 = text_embeddings[:,-1:]
        text_embeddings = torch.cat((text_embeddings_0, text_embeddings_1), dim=1).contiguous()
    elif in_channels == 3:
        text_embeddings_0 = text_embeddings[:,0:1]
        text_embeddings_1 = text_embeddings[:,-2:]
        text_embeddings = torch.cat((text_embeddings_0, text_embeddings_1), dim=1).contiguous()
...
def get_minicpm_inputs_embeds(videos=None, images=None, audios=None, text_prompt=None, proj=minicpm_proj, in_channels=minicpm_in_channels):
    ...
    output_hidden_state = minicpm_encoder.generate(**inputs,\
        tokenizer=minicpm_tokenizer,max_new_tokens=2048,decode_text=False).hidden_states
